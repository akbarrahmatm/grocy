...
export default function Checkout() {
  ...
  useEffect(() => {
    ...
    addressApi
      ...
      .catch((err) =>
        push(err instanceof Error ? err.message : "Failed to load addresses", "error")
      );
    return () => {
      cancelled = true;
    };
  ...
  async function placeOrder() {
    ...
    try {
      const order = await orderApi.create({
        ...
        ...(selected ? { courier: { code: selected.code, service: selected.service } } : {}),
      });
      window.location.assign(order.snap_redirect_url ?? `/orders/${order.id}/complete`);
    } catch (err) {
      push(err instanceof Error ? err.message : "Failed to place order", "error");
      setPlacing(false);
    }
  }
    ...
    <div className="phone">
      ...
      <section className="px-5 pt-2">
        ...
        {!addresses ? (
          ...
          <div className="address-list" role="radiogroup" aria-label="Delivery address">
            {addresses.map((a) => (
              <button
                type="button"
                key={a.id}
        ...
        <section className="px-5 pt-5">
          ...
          {chosenAddress && !chosenAddress.destination_id ? (
            <p className="text-sm text-[var(--coral)] py-2">
              This address has no delivery area. Edit it and pick one to see shipping rates.
            </p>
          ) : ratesLoading ? (
            <p className="text-sm text-[var(--ink-soft)] py-2">Checking rates…</p>
          ) : ratesError ? (
            <div className="py-2">
              <p className="text-sm text-[var(--coral)]">{ratesError}</p>
              <p className="text-xs text-[var(--ink-soft)] mt-1">
                You can still place the order without shipping and arrange delivery later.
              </p>
            </div>
          ) : rates.length === 0 ? (
            <p className="text-sm text-[var(--ink-soft)] py-2">Select an address.</p>
          ) : (
      ...
      <section className="px-5 pt-5">
        <label htmlFor="checkout-note" className="block text-sm font-medium mb-1.5 text-[var(--ink)]">
        ...
        <div className="summary">
          ...
          {SHOW_SHIPPING && (
            <div className="summary-row">
              <span>Shipping</span>
              <span>{selected ? formatCurrency(selected.price) : formatCurrency(0)}</span>
            </div>
          )}
      ...
      <BottomNav active="cart" cartCount={items.reduce((s, it) => s + it.qty, 0)} />
    </div>
  );
}
